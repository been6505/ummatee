...
function doPost(e) {
  try {
    ...
    // --- แจ้งสถานะคำสั่งซื้อไปหาลูกค้าทาง LINE ---
    //
    // ความปลอดภัย: ห้ามรับ lineUserId/ข้อความ จากผู้เรียกเด็ดขาด — SHEET_TOKEN เป็น token ฝั่ง client
    ...
    //
    // จึงรับแค่ orderId + event แล้วมา "อ่านเบอร์ผู้รับจากตัวออเดอร์เอง" ผ่าน Firestore REST
    // (orders มี allow get: if true อ่านแบบไม่ต้องล็อกอินได้) และประกอบข้อความจากเทมเพลตในนี้เท่านั้น
    // ผลคือ: ต่อให้รู้ token ก็ส่งได้แค่ข้อความมาตรฐานไปหาลูกค้าเจ้าของออเดอร์นั้นเอง (ต้องรู้ orderId
    // ซึ่งเป็น random id 20 ตัวอักษร เดาไม่ได้) ส่งหาคนอื่นหรือเขียนข้อความเองไม่ได้อีกต่อไป
    if (data.type === 'lineNotify') {
      ...
      if (!data.orderId || !data.event) {
        return ContentService.createTextOutput(JSON.stringify({ error: 'missing orderId/event' }))
          .setMimeType(ContentService.MimeType.JSON)
      }
      ...
      if (!lineUserId) {
        return ContentService.createTextOutput(JSON.stringify({ ok: false, reason: 'no lineUserId on order' }))
          .setMimeType(ContentService.MimeType.JSON)
      }
      ...
      if (!text) {
        return ContentService.createTextOutput(JSON.stringify({ error: 'unknown event' }))
          .setMimeType(ContentService.MimeType.JSON)
      }
